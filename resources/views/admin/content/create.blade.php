        ...
        <div class="col-12">
            ...
            <div class="block">
                <div class="block-header block-header-default">
                    <h3 class="block-title">{{ __('Add content') }}</h3>
                </div>
                <div class="block-content">
                    <form action="{{ route('admin.contents.store', ['locale' => app()->getLocale()]) }}" method="post">
                        @csrf
                        <div class="form-group row">
                            <div class="col-md-6 {{ $errors->has('content.key') ? 'is-invalid' : '' }}">
                            ...
                            <div class="col-md-6 {{ $errors->has('content.page_id') ? 'is-invalid' : '' }}">
                                <div class="form-material floating">
                                    <select class="form-control"
                                        ...
                                        @foreach($pages as $page)
                                            <option value="{{ $page->id }}"
                                                {{ old("content.page_id") == $page->id ? 'selected' : '' }}
                                            >{{ $page->label }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="form-group row">
                            @foreach($langs as $lang)
                                <div class="col-md-6 mb-4">
                                    ...
                                    <div class="{{ $errors->has("text.$loop->index.link") ? 'is-invalid' : '' }}">
                                        <div class="form-material floating">
                                            <input type="text"
                                                   class="form-control"
                                                   id="link-{{ $loop->index }}"
                                                   name="text[{{ $loop->index }}][link]"
                                                   autocomplete="off"
                                                   value="{{ old("text.$loop->index.link") }}">
                                            <label for="link-{{ $loop->index }}">
                                                {{ __('Link') }}
                                            </label>
                                            @error("text.$loop->index.link")
                                            <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    </div>
                        ...
                        <div class="form-group row">
                            <div class="col-md-12 text-center my-3">
                                ...
                                <a href="{{ route('admin.contents.index', ['locale' => app()->getLocale()]) }}"
                                   type="submit"
