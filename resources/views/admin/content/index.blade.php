...
@section('content')
    <!-- Page Content -->
    <div class="content">
        <h2 class="content-heading">{{ __('Content') }}</h2>
        <div class="block">

            <div class="block-header block-header-default">
                <div class="col-sm-6 col-xl-4">
                    <a href="{{ route('admin.contents.create', ['locale' => app()->getLocale()]) }}" type="button"
                       class="btn btn-alt-secondary min-width-125 js-click-ripple-enabled"
                       data-toggle="click-ripple"
                       style="overflow: hidden; position: relative; z-index: 1;">{{ __('Add content') }}</a>
                </div>
            </div>
                ...
                <table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                    ...
                    @foreach($contents as $content)
                        <tr>
                            ...
                            <td class="font-w600 text-center">{!! $content->text !!}</td>
                            <td class="d-none d-sm-table-cell text-center">{{ $content->page_label }}</td>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <a href="{{ route('admin.contents.edit', ['locale' => app()->getLocale(), 'content' => $content->id]) }}"
                                       class="btn btn-sm btn-primary js-tooltip-enabled"
                                       data-toggle="tooltip" title="{{ __('Edit') }}" data-original-title="Edit">
                                        <i class="fa fa-pencil"></i>
                                    </a>
                                    <form
                                        action="{{ route('admin.contents.destroy', ['locale' => app()->getLocale(), 'content' => $content->id]) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
